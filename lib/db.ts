...
export async function initDb() {
  ...
  // Add product_type column if it doesn't exist (for existing databases)
  ...
  try {
    ...
    // SQLite will throw an error if column exists, which is fine
  }
...
export async function saveTemplate(template: Omit<Template, 'created_at' | 'updated_at'>) {
  await dbRun(
    `INSERT INTO templates (id, name, subject, body, updated_at) 
    ...
    [template.id, template.name, template.subject || null, template.body]
  );
}
